...
function action(   act, i, from, to, saying, chance, token, n, tmp, plugin, command) {
  dbg(6, "action", sprintf("processing %d actions", sysvar["actions"]))
  ...
  for (act=1; act<=sysvar["actions"]; act++) {
    ...
    if (split(actions[act], token, "\001") == 6) {
      ...
      if (tolower(token[1]) == tolower(var["action"])) {
          ...
          for (i=1; i<=n; i++) {
            ...
            if (substr(tmp[i], 1, 1) == "%") {
              ...
              if (isPartOf(var["user"], substr(tmp[i], 2))) {
                from = 1
                dbg(6, "action", sprintf(" FROM (%s in %s)", var["user"], tmp[i]))
                break
              }
        ...
        if (token[3]) {
          ...
          for (i=1; i<=n; i++) {
            ...
            if (tolower(var["target"]) == tmp[i]) {
              to = 1
              dbg(6, "action", sprintf(" TO (%s)", var["target"]))
              break
            }
          }
        } else to = 1
        ...
        if (token[4]) {
          ...
          if (tolower(var["msg"]) ~ tolower(token[4])) {
            saying = 1
            dbg(6, "action", sprintf(" SAYING (%s)", var["msg"]))
          }
        } else saying = 1
        ...
        # CHANCE <percent>
        if (token[5]) {
          # generate integer 1-100 and check if CHANCE is bigger (or equal)
          rnd = int(rand() * 100) + 1
          if (token[5] >= rnd)
            chance = 1
        ...
        if (from && to && saying && chance) {
          dbg(4, "action", sprintf("raw \"%s\" matches action \"%s\"", var["raw"], reaction(actions[act])))
          ...
          if ( (i=index(command, " ")) > 0) {
            var["cmd"] = substr(command, 1, i-1)
            var["args"] = substr(command, i+1)
          } else {
            var["cmd"] = substr(command, 1)
            var["args"] = ""
          }

          dbg(5, "action", sprintf("plugin: \"%s\" command: \"%s\" args: \"%s\"", plugin, var["cmd"], var["args"]))

          # pick plugin for command to  run
          switch(plugin) {
            # "raw" just send command to IRC server
            case "raw":
              send(ircd, vsub(command))
              return
            break
            ...
            case "cmd": 
              if (var["cmd"] in commands) {
                ...
                out = @call(var["args"])
                if (out) send(ircd, vsub("PRIVMSG $T :"out))
                  if (out ~ /^ACTION/)
                    send(ircd, vsub(sprintf("PRIVMSG $T :\001%s\001", out)))
                  else
                    send(ircd, vsub(sprintf("PRIVMSG $T :%s", out)))
              } else dbg(4, "main", sprintf("No such internal function \"%s\"", var["cmd"]))
              return
            break
            ...
            case "awk":
              ...
              n = dyncommand("awk", var["cmd"]".awk", var["args"], output)
              # send output as PRIVMSG to IRC server
              for (i=1; i<=n; i++)
                if (output[i] ~ /^ACTION/)
                  send(ircd, vsub(sprintf("PRIVMSG $T :\001%s\001", output[i])))
                else
                  send(ircd, vsub(sprintf("PRIVMSG $T :%s", output[i])))
              return
            break
        ...
        } else dbg(5, "action", sprintf("%s Failed%s%s%s%s", reaction(actions[act]), from?"":" FROM", to?"":" TO", saying?"":" SAYING", chance?"":" CHANCE"))
      }
